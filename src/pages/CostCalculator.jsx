...
const CostCalculator = () => {
    ...
    const [userData, setUserData] = useState({
        ...
        'entry.411785020': '',
        'entry.2060475251': '',
        'entry.1158509870': estimatedCost,  // Estimated cost value
        'entry.620862685': formData.cleaningType,  // Cleaning type
        'entry.467381148': formData.addOns.join(', '),  // Add-ons as a comma-separated string
    });
    ...
    const handleFormSubmit = (e) => {
        ...
        const googleFormBaseURL = `https://docs.google.com/forms/d/e/1FAIpQLSdgy_xkMC0xfrI1HRb135Wy586vqg_XdbwVC8iiXpFsYGVBSQ/viewform?usp=pp_url`;

        // Constructing the prefilled URL with the user input data
        const prefilledURL = `${googleFormBaseURL}&entry.2021383152=${encodeURIComponent(userData['entry.2021383152'])}&entry.821350656=${encodeURIComponent(userData['entry.821350656'])}&entry.411785020=${encodeURIComponent(userData['entry.411785020'])}&entry.2060475251=${encodeURIComponent(userData['entry.2060475251'])}&entry.1158509870=${encodeURIComponent(estimatedCost)}&entry.620862685=${encodeURIComponent(formData.cleaningType)}&entry.467381148=${encodeURIComponent(formData.addOns.join(', '))}`;

        // Redirect the user to the prefilled Google Form URL
        window.open(prefilledURL, "_blank");
    };

    // https://docs.google.com/forms/d/e/1FAIpQLSdgy_xkMC0xfrI1HRb135Wy586vqg_XdbwVC8iiXpFsYGVBSQ/viewform?usp=pp_url&entry.2021383152=name&entry.2060475251=email&entry.821350656=contact&entry.411785020=address&entry.1158509870=estimate&entry.620862685=service&entry.467381148=addon
...

            {
                ...
                <>
                    ...
                    <form method='POST' onSubmit={handleFormSubmit} className='flex flex-col gap-4 md:w-1/2 mt-10'>
                        ...
                        <input
                            type='text'
                            name='entry.2021383152'
                            value={userData['entry.2021383152']}
                        ...
                        <input
                            type='text'
                            name='entry.821350656'
                            value={userData['entry.821350656']}
                            onChange={userDataChange}
                        ...
                        <input
                            type='email'
                            name='entry.2060475251'
                            value={userData['entry.2060475251']}
                            onChange={userDataChange}
                            placeholder='Email'
                            className='outline outline-2 outline-slate-300 focus:border-none focus:outline-blue-900 py-2 px-5 rounded-lg duration-300'
                        />
                        ...
                        <input
                            type='text'
                            name='entry.411785020'
                            value={userData['entry.411785020']}
                            onChange={userDataChange}
                            placeholder='Address'
                            className='outline outline-2 outline-slate-300 focus:border-none focus:outline-blue-900 py-2 px-5 rounded-lg duration-300'
                        />
